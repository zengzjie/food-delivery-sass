import { RedisService as NRedisService } from '@liaoliaots/nestjs-redis';
...
export class RedisService {
  ...
  constructor(private readonly redisService: NRedisService) {
   ...
   * @param key
   */
  async lLeftPop(key: string): Promise<any> {
    if (!key) return null;
   ...
   * @param key
   */
  async lRightPop(key: string): Promise<any> {
    if (!key) return null;
